...
export const RegisterPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex relative overflow-hidden" style={{ backgroundColor: '#0a150e' }}>
      {/* Background Effects */}
      <div className="fixed inset-0 pointer-events-none">
        ...
        <div className="absolute w-[600px] h-[600px] rounded-full blur-[120px] opacity-15"
          style={{ top: '10%', left: '20%', backgroundColor: 'rgba(74,222,128,0.25)' }} />
        <div className="absolute w-[500px] h-[500px] rounded-full blur-[120px] opacity-10"
          style={{ bottom: '20%', right: '30%', backgroundColor: 'rgba(30,74,40,0.4)' }} />
      </div>

      {/* Left Panel - Branding */}
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden">
        <div className="absolute inset-0" style={{ background: 'linear-gradient(to bottom right, rgba(74,222,128,0.08), #0a150e, #0a150e)' }} />
        ...
        <div className="relative z-10 flex flex-col justify-center px-16 xl:px-24 w-full">
          ...
          <div className={`flex items-center gap-4 mb-16 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-12'}`}>
            <div className="relative">
              <div className="absolute inset-0 rounded-2xl blur-xl opacity-50" style={{ background: 'linear-gradient(to bottom right, #4ade80, #2d6a3f)' }} />
              <div className="relative w-16 h-16 rounded-2xl flex items-center justify-center shadow-2xl" style={{ background: 'linear-gradient(to bottom right, #4ade80, #3d8a55)' }}>
                <TreePine className="w-8 h-8" style={{ color: '#0a150e' }} />
              </div>
            </div>
            <span className="text-4xl font-bold" style={{ background: 'linear-gradient(to right, #4ade80, #86efac)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Acorn</span>
          </div>
          ...
          <div className={`space-y-4 transition-all duration-1000 delay-500 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-12'}`}>
            {trustBadges.map((badge, index) => (
              <div
                ...
                style={{
                  backgroundColor: 'rgba(30,74,40,0.25)',
                  border: '1px solid rgba(74,222,128,0.2)',
                  transitionDelay: `${index * 100}ms`
                }}
              >
                <div className="w-12 h-12 rounded-xl flex items-center justify-center"
                  style={{ background: 'linear-gradient(to bottom right, rgba(74,222,128,0.2), rgba(45,106,63,0.1))' }}>
        ...
        <div className="w-full max-w-md">
          ...
          <div className="lg:hidden text-center mb-8">
            <Link to="/" className="inline-flex items-center gap-3">
              <div className="w-12 h-12 rounded-xl flex items-center justify-center" style={{ background: 'linear-gradient(to bottom right, #4ade80, #3d8a55)' }}>
                <TreePine className="w-6 h-6" style={{ color: '#0a150e' }} />
              </div>
              <span className="text-2xl font-bold" style={{ background: 'linear-gradient(to right, #4ade80, #86efac)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Acorn</span>
            </Link>
          </div>
                ...
                <React.Fragment key={s}>
                  <div className="flex items-center gap-2">
                    <div
                      ...
                      style={{
                        background: step >= s ? 'linear-gradient(to bottom right, #4ade80, #3d8a55)' : '#1a3520',
                        color: step >= s ? '#0a150e' : '#6b9e7a',
                        boxShadow: step >= s ? '0 10px 25px -5px rgba(74,222,128,0.3)' : 'none'
                      }}
                    >
                  ...
                  {s < 2 && (
                    <div
                      ...
                      style={{ backgroundColor: step >= 2 ? '#4ade80' : '#1a3520' }}
                    />
                  )}
                </React.Fragment>
              ))}
              ...
              {step === 1 && (
                <div className="space-y-5">
                  ...
                  <Button
                    ...
                    style={{
                      background: 'linear-gradient(to right, #4ade80, #3d8a55)',
                      color: '#0a150e',
                          ...
                          {[
                            ...
                            <div key={key} className="flex items-center gap-2">
                              <div
                                ...
                                style={{
                                  backgroundColor: passwordChecks[key as keyof typeof passwordChecks] ? '#4ade80' : '#1a3520',
                    ...
                    >
                      {isLoading ? (
                        <div className="flex items-center justify-center gap-2">
                          <div className="w-5 h-5 border-2 border-[#0a150e] border-t-transparent rounded-full animate-spin" />
                          Creating...
                        </div>
                      ) : (
                        <>
